...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./byteformer_gpt2_prefix_caption", help="训练输出目录")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            prefix_len = prefix_embeds.size(1)  # prefix的序列长度
            label_len = labels.size(1)
            prediction_logits = logits[:, prefix_len:prefix_len+label_len, :]  # 跳过prefix位置，取caption部分
            # 将labels中的pad token替换为-100
            pad_token_id = getattr(self.decoder_tokenizer, 'pad_token_id', None)
            shift_labels = labels.clone()
            if pad_token_id is not None:
...
def main():
    ...
    def compute_metrics(pred):
        ...
        # 处理pred_ids的形状，确保是二维数组
        if len(pred_ids.shape) == 1:
            pred_ids = pred_ids.reshape(1, -1)
        if len(labels_ids.shape) == 1:
            labels_ids = labels_ids.reshape(1, -1)
    ...
    class PrefixCompatibleTrainer(MySeq2SeqTrainer):
        def evaluate(self, eval_dataset=None, desc="Eval", ignore_keys=None, metric_key_prefix: str = "eval"):
            ...
            for batch in eval_iterator:
                ...
                with torch.no_grad():
                    ...
                    for step in range(max_length - 1):  # 最多生成max_length-1个新token
                        ...
                        # 在第一个batch的第一步添加调试信息
                        if len(predictions) == 0 and step == 0:
                            print(f"[DEBUG] Step {step}: next_token_logits shape: {next_token_logits.shape}")
                            print(f"[DEBUG] Step {step}: next_token: {next_token[0].item()}")
                            print(f"[DEBUG] Step {step}: decoded token: '{tokenizer.decode([next_token[0].item()])}'")
                        
                        # 检查是否所有序列都生成了EOS token
                        if (next_token == tokenizer.eos_token_id).all():
                            if len(predictions) == 0:  # 只在第一个batch时打印
                                print(f"[DEBUG] EOS reached at step {step}")
                            break
            ...
            if predictions and self.compute_metrics:
                pred = type('Pred', (), {})()
                pred.predictions = np.array(predictions)
                pred.label_ids = np.array(references)
                metrics = self.compute_metrics(pred)
                print(f"[{desc}] Loss: {avg_loss:.4f}, Metrics: {metrics}")
                return avg_loss, metrics
            else:
